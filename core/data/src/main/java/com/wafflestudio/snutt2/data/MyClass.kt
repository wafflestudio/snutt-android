...
fun DeleteFirebaseTokenResults.toTempModel() = DeleteFirebaseTokenResultsT(
    message = this.message,
)

fun DeleteTableResults.toTempModel() = this.map { it.toTempModel() }

fun DeleteUserAccountResults.toTempModel() = DeleteUserAccountResultsT(
    message = this.message,
)

fun DeleteUserFacebookResults.toTempModel() = DeleteUserFacebookResultsT(
    token = this.token,
)
...
fun GetBookmarkListResults.toTempModel() = GetBookmarkListResultsT(
    year = this.year,
    semester = this.semester,
    lectures = this.lectures.map { it.toTempModel() },
)

fun GetCoursebooksOfficialResults.toTempModel() = GetCoursebooksOfficialResultsT(
    url = this.url,
)

fun GetLecturesIdResults.toTempModel() = GetLecturesIdResultsT(
    id = this.id,
)

fun GetNotificationCountResults.toTempModel() = GetNotificationCountResultsT(
    count = this.count,
)
...
fun GetUserFacebookResults.toTempModel() = GetUserFacebookResultsT(
    name = this.name,
...
fun PatchUserInfoParams.toTempModel() = PatchUserInfoParamsT(
    nickname = this.nickname,
)
...
fun PostCheckEmailByIdParams.toTempModel() = PostCheckEmailByIdParamsT(
    id = this.id,
)

fun PostCheckEmailByIdResults.toTempModel() = PostCheckEmailByIdResultsT(
    email = this.email,
)

fun PostCustomLectureParams.toTempModel() = PostCustomLectureParamsT(
    id = this.id,
    classification = this.classification,
    department = this.department,
    academic_year = this.academic_year,
...
fun PostFeedbackResults.toTempModel() = PostFeedbackResultsT(
    message = this.message,
)
...
fun PostForceLogoutParams.toTempModel() = PostForceLogoutParamsT(
    userId = this.userId,
    registrationId = this.registrationId,
)

fun PostForceLogoutResults.toTempModel() = PostForceLogoutResultsT(
    message = this.message,
)
...
fun PostLoginFacebookParams.toTempModel() = PostLoginFacebookParamsT(
    facebookId = this.facebookId,
    facebookToken = this.facebookToken,
)

fun PostLoginFacebookResults.toTempModel() = PostLoginFacebookResultsT(
    token = this.token,
    userId = this.userId,
)

fun PostResetPasswordParams.toTempModel() = PostResetPasswordParamsT(
    id = this.id,
    password = this.password,
)

fun PostSearchQueryParams.toTempModel() = PostSearchQueryParamsT(
    year = this.year,
    semester = this.semester,
    title = this.title,
    classification = this.classification,
...
fun PostSendCodeToEmailParams.toTempModel() = PostSendCodeToEmailParamsT(
    email = this.email,
)

fun PostSendPwResetCodeParams.toTempModel() = PostSendPwResetCodeParamsT(
    email = this.email,
)
...
fun PostUserFacebookParams.toTempModel() = PostUserFacebookParamsT(
    facebookId = this.facebookId,
    facebookToken = this.facebookToken,
)

fun PostUserFacebookResults.toTempModel() = PostUserFacebookResultsT(
    token = this.token,
)

fun PostUserPasswordParams.toTempModel() = PostUserPasswordParamsT(
    id = this.id,
    password = this.password,
)

fun PostUserPasswordResults.toTempModel() = PostUserPasswordResultsT(
    token = this.token,
)

fun PostVerifyEmailCodeParams.toTempModel() = PostVerifyEmailCodeParamsT(
    code = this.code,
)

fun PostVerifyPwResetCodeParams.toTempModel() = PostVerifyPwResetCodeParamsT(
    id = this.id,
    code = this.code,
)
...
fun PutTableThemeParams.toTempModel() = PutTableThemeParamsT(
    theme = this.theme,
    themeId = this.themeId,
)

fun PutUserPasswordParams.toTempModel() = PutUserPasswordParamsT(
    newPassword = this.newPassword,
    oldPassword = this.oldPassword,
)

fun PutUserPasswordResults.toTempModel() = PutUserPasswordResultsT(
    token = this.token,
)

fun RegisterFirebaseTokenParams.toTempModel() = RegisterFirebaseTokenParamsT()

fun RegisterFirebaseTokenResults.toTempModel() = RegisterFirebaseTokenResultsT(
    message = this.message,
)
