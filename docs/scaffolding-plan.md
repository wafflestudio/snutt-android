...
│   └── util/                        UI 작성 헬퍼 (dp/sp 변환, 도메인→표시 문자열 등)
│
├── navigation/                      앱 라우팅 구조
...
- `util/` — UI 작성에 쓰이는 헬퍼 (dp 변환, 도메인 모델 → 표시 문자열 포매터 등)
...
UI 헬퍼들을 일단 `ui/util/` 한 곳에 모았지만, 파일 자체가 파편화되어 있다. 나중에 파일 내용을
하나씩 보면서 다음을 정리한다:

- **`SNUTTStringUtils` + `SNUTTStringUtilsNew` 통합** — 같은 역할의 두 object로 나눠져 있음
- **`ModelExt.kt` 분해** — 도메인 확장, DTO 오퍼레이션, UI 색 매핑, URL 포매팅이 한 파일에
  혼재
- **파일명 규칙 통일** — `*Util.kt`, `*Ext.kt`, `SNUTT*.kt` 혼재
- **`SNUTTUtils.kt` 분해** — 시간/요일 포맷, dp 변환, 화면 크기, 네트워크 체크가 한 object에
  섞여 있음
