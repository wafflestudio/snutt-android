...
@HiltViewModel
class ThemeConfigViewModelNew @Inject constructor(
    ...
    init {
        viewModelScope.launch {
            themeRepository.fetchThemesNew()
        }
    ...
    fun onClickApply(theme: TableTheme) {
        viewModelScope.launch {
            ...
            val currentTableId = currentTableRepository.currentTable.value?.id ?: return@launch
            when (theme) {
                is CustomTheme -> tableRepository.updateTableThemeNew(currentTableId, theme.id)
                is BuiltInTheme -> tableRepository.updateTableThemeNew(currentTableId, theme.code)
    ...
    fun onClickDuplicate(theme: CustomTheme) {
        viewModelScope.launch {
            ...
            _uiEvent.emit(ThemeConfigUiEvent.CloseBottomSheet)
            themeRepository.copyThemeNew(theme.id)
                .onFailure { handleError(it) }
        }
    }
    ...
    fun onConfirmDeleteTheme() {
        ...
        viewModelScope.launch {
            ...
            themeRepository.deleteThemeNew(theme.id)
                .onSuccess {
                    val currentTable = currentTableRepository.currentTableRefactored.value ?: return@onSuccess
                    val isApplied = (currentTable.themeRef as? ThemeReference.Custom)?.themeId == theme.id
                    if (isApplied) {
                        tableRepository.fetchTableByIdNew(currentTable.summary.id)
