...
@HiltViewModel
class PushPreferencesViewModel @Inject constructor() : ViewModel() {
    ...
    private val _pushPreferencesUiEvent: MutableSharedFlow<String> = MutableSharedFlow()
    val pushPreferencesUiEvent = _pushPreferencesUiEvent.asSharedFlow()

    fun loadPushPreferences() {
        viewModelScope.launch {
            runCatching {
                _pushPreferencesUiState.emit(PushPreferencesUiState.Success(PushPreferences(lectureUpdate = false, vacancyNotification = false))) // TODO: temp
            }.onFailure { e ->
