...
class TableView : View {
    ...
    interface Injector {
        ...
        fun getLectureManager(): LectureManager
    }
    ...
    fun inject() {
        val injector = EntryPoints.get(context.applicationContext, Injector::class.java)
        lectureManager = injector.getLectureManager()
        prefStorage = injector.getPrefStorage()
    }

    constructor(context: Context) : super(context) {
        inject()
        export = true
        val table = prefStorage.currentTable
        table?.lectureList?.let {
            lectureManager.setLectures(it)
        }
    ...
    private fun init() {
        ...
        wdays[0] = context.resources.getString(R.string.wday_mon)
        wdays[1] = context.resources.getString(R.string.wday_tue)
        wdays[2] = context.resources.getString(R.string.wday_wed)
        wdays[3] = context.resources.getString(R.string.wday_thu)
        wdays[4] = context.resources.getString(R.string.wday_fri)
        wdays[5] = context.resources.getString(R.string.wday_sat)
        wdays[6] = context.resources.getString(R.string.wday_sun)
    ...
    private fun drawTimetable(canvas: Canvas, canvasWidth: Int, canvasHeight: Int, export: Boolean) {
        ...
        for (lecture in lectures!!) {
            for (classTime in lecture.class_time_json) {
                ...
                val start = classTime.start
                val duration = classTime.len
        ...
        if (!export && lec != null) {
            for (classTime in lec.class_time_json) {
                val wday = classTime.day
                val start = classTime.start
                val duration = classTime.len
                startWday = Math.min(startWday, wday)
    ...
    private val tabBarHeight: Int
        private get() {
            var tabBarHeight = 0
            tabBarHeight = context.resources.getDimensionPixelSize(R.dimen.tab_bar_height)
            return tabBarHeight
        }
    private val activity: SNUTTBaseActivity
        get() = context as SNUTTBaseActivity
